...
module.exports = (stream, width, heigth, cusotmChars) => new Promise((resolve, reject) => {
  let ASCIIString = '';
  ...
  const transformer = sharp()
    ...
    .toColorspace('srgb')
    // set white backround for next action
    .background({
      r: 255,
      g: 255,
      b: 255,
  ...
  transformer.on('data', (chunk) => {
    ...
    for (let i = 0; i < chunk.length; i += 3) {
      ...
      // TODO: before having all these variables it looked faster, do bechmark
      const r = chunk[i];
      const g = chunk[i + 1];
      const b = chunk[i + 2];
      // const a = channels === 4 ? chunk[i + 3] : false;
      const char = getChar(r, g, b, false || cusotmChars);
      ASCIIString += char;
    }
  });
  ...
  transformer.on('info', (info) => {
    ...
    transformer.on('end', () => {
      for (let i = info.width; i < ASCIIString.length; i += info.width + 1) {
        // add new lines at the specfied width
